...
当前状态（2026-09-23）：**注册隔离证据传输纠正已交付**。`25e0116`已部署既有测试环境，本地/CI/Docker完整CTest均115/115；部署前真实回读及部署后独立验收通过，88份文件含两份隐藏锁文件完整匹配，Smoke与固定工程回归成功。仅一次纠正发布，新阶段READY，旧缺件失败和三个HALTED均保留。离线学习/时钟/隔离拒绝能力有工程证据，但线上策略配置未改，运行仍PASS_WITH_ACTIONS、执行资格未评估；旧研究/候选关闭，数据与盈利资格仍不通过。不默认等待两周或重复这条部署修复链。详见[本批交付与下一节奏](docs/reviews/2026-09-23-evidence-transport-correction-result.md)和[当前状态](docs/CURRENT_STATE.md)。离线使用与数据保存见[封版说明](docs/OFFLINE_ENGINEERING_FREEZE.md)。下方“已覆盖”仅指需求文档覆盖，不代表盈利或运行验收通过。
...
- 强平距离（单仓最小值）：<8% 只减仓；有持仓但风险数据未知时禁止加仓；P95 仅作历史诊断
