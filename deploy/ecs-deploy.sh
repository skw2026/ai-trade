...
# Keep 128 MiB above the post-pull target while allowing the pre-mutation
# image pull to determine its real footprint under the transaction guard.
...
cleanup_deploy_host_storage() {
  ...
  if (( DEPLOY_OPTION_LIFECYCLE_V3_CAPTURE_RETENTION_HOURS < 960 )); then
  ...
  if ! python3 "${capture_pruner}" \
      --root "${DEPLOY_RELEASE_ROOT}/data/research/bybit_btc_option_lifecycle_v3" \
...
reclaim_research_capture_for_transaction() {
  ...
  if [[ ! "${DEPLOY_PRESSURE_OPTION_LIFECYCLE_V3_CAPTURE_RETENTION_HOURS}" =~ ^[0-9]+$ ]] ||
