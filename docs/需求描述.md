...
- 杠杆白名单：100%（越界即FAIL）
- 强平距离（2026-09-05 金融语义修订）：按每个持仓的最小距离保护，不使用加权 P95 放行。
  - 任一持仓距离 < 8%：只减仓并审计，不能被安全仓位掩盖；目标恢复 ≥ 10%。
  - 有持仓但 mark/liq 缺失或非有限：禁止加仓；P95 仅保留作历史诊断。
  - 回撤 Fuse/Cooldown 的目标归零及其滞回不能被断连/只减仓状态覆盖。
...
- 同一候选必须以 `model_version + model_sha256 + git_commit` 贯穿训练、replay、
  registry 和 live，不允许跨版本拼接证据。
- 离线收益必须来自不重叠经济曲线；所有变换只在 train 拟合，边界按标签 horizon
  purge/embargo。
- 模型晋升必须同时通过独立 holdout 的成本后经济性和当前候选 replay；AUC/IC 仅作
  诊断，不能豁免净收益失败。
- canary 必须能产生受限模型动作并积累真实去重成交；0 applied 或 0 fills 只能判定为
  `EVIDENCE_INSUFFICIENT`。
- 在线自适应必须以前段选候选、后段 holdout 验收；样本不足冻结，禁止原位选优。
- 详细状态机、硬停止条件和晋升定义见 `docs/闭环收敛契约.md`。
