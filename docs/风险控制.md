...
- DD≥20%：进入FUSE（全平 + 24h冷却建议 + 25%预算分段恢复）
要求：
- 触发后60s内状态生效并审计
...
- **合规过滤器 (Exchange Filters)**: 下单前必须通过交易所规则校验，否则直接丢弃或修正：
  - `min_notional`: 若名义价值 < 5U (Binance)，直接忽略（防尘埃单拒单）。
...
### 3.1 订单状态机（最小集）
NEW → SENT → ACK → PARTIAL/FILLED → CANCELLED
REJECTED（可从SENT/ACK进入）
