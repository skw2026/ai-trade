...
### 1.3 强平距离（单仓最小值，P95 仅作历史诊断）

- 多：(mark-liq)/mark；空：(liq-mark)/mark；任一有仓位合约低于阈值即只减仓，不能被安全仓位的名义权重掩盖。
- 任一持仓缺少有效 mark/liq 时禁止加仓，并记录 `RISK_LIQUIDATION_DATA_UNKNOWN`；空仓与未知风险分开。
- 配置 `risk.liquidation.min_distance` 的默认阈值仍为 8%；旧键 `min_distance_p95` 作为兼容别名保留，但不再使用上尾分位数作安全门。
- 回撤 Fuse/Cooldown 的目标归零优先于只减仓；回撤滞回独立保存，断连或人工只减仓不能擦除。
- 模拟/回放若没有强平风险证据，也不得默认为安全；应标明输入不完整，不能伪造一个价格恢复交易。
...
- S5 默认把策略性平仓与风控强平拆开：policy-flat / 反手先平旧仓属于策略性 Reduce，优先 PostOnly maker；COOLDOWN/REDUCE_ONLY/FUSE 等风控强制收敛与 SL/TP 保护单仍走 taker，优先保证风险退出。
- 超时撤单重挂：max_order_age 到期撤单并重新挂
- **合规过滤器 (Exchange Filters)**: 下单前必须通过交易所规则校验，否则直接丢弃或修正：
  - `min_notional`: 若名义价值低于交易所该 symbol 最小名义金额，直接忽略并审计 `EXEC_FILTER_IGNORE`。
    - **修正逻辑**: 若 `abs(TargetPosition) < min_notional`，则强制将 `TargetPosition` 修正为 0。避免策略持续发出“买不起”的微小仓位指令导致死循环。
    - **例外（FUSE/强制平仓）**: 若意图是平仓 (Target=0) 且持仓本身低于该 symbol 最小名义金额（尘埃仓位），不得判定为熔断失败；改为记录 `EXEC_DUST_POSITION_STUCK`，并将该仓位置为 `DUST_LOCKED`（只允许净减，不允许任何加仓）。
...
- 降级规则：
  ...
  - 保护单触发的 `REDUCE_ONLY` 不是永久锁：仅当账户重新回到空仓、无 managed protection、无待确认 required SL、无在途净仓位订单时，系统自动释放并审计 `PROTECTION_FORCE_REDUCE_ONLY_RELEASED`。
  - 若仍有任何敞口、保护单状态或在途净仓位订单，保护单 `REDUCE_ONLY` 必须继续保持，避免在保护链路未闭环时重新开仓。
...
- 对账异常恢复：
  - 连续异常达到阈值后可进入 anomaly reduce-only / halt
  - 若账户本地净敞口为 0 且无净仓位在途订单，远端快照暂不可用不再继续放大异常；系统记录 `OMS_RECONCILE_DEGRADED_FLAT_IDLE` 并累计健康窗口
  - 连续健康窗口达到 `anomaly_resume_streak` 后，系统自动退出 anomaly reduce-only / halt，并记录 `OMS_RECONCILE_ANOMALY_PROTECTION_EXIT` / `OMS_RECONCILE_ANOMALY_HALT_EXIT`
- 撤单/成交竞态处理：
  - `Cancelled` 订单仍可能收到交易所 late fill，OMS 必须记录成交量并补齐净成交基线
  - 若账户仓位已通过远端同步提前反映该 fill，本地不得再次应用仓位变化，只补齐 OMS 并审计 `FILL_ACCOUNT_ALREADY_REFLECTED`
  - 报告需统计 `fill_account_already_reflected_count` 与 `fill_cancelled_order_applied_count`，用于验证对账异常是否被源头消除
...
D. Risk（Hard）
- 回撤阈值状态机（8/12/20）
- 强平距离阈值（任一持仓最小距离<8%只减仓；未知风险禁止加仓）
- 杠杆白名单+逐仓强制
...
### 6.4 阶段3（因子-模型-风控闭环前两级：Miner + Integrator）
- Miner（离线）：符号回归/遗传规划产出因子表达式集（可复现、可审计）
- Integrator（离线训练 + 在线影子）：CatBoost 时序训练与影子推理（不直连下单）
- Regime/Gate/Risk 保持硬保险栓：模型输出仅影响信号层，不可覆盖硬风控
- 影子验证通过后，按灰度方式接入真实决策路径

> 说明：SOL 参数分化与专项 Gate 指标可作为并行优化项，不再作为阶段3主定义。
