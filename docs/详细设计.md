...
约束：
- `FactorDefinition` 默认只进入审计/训练链路；`ModelScore` 按
  `off/shadow/canary/active` 模式进入对应决策链路；
- 接入真实交易决策前，必须通过 Gate 的 R1/R2/R3 阶段验收；
- `ModelScore` 不得绕过 `RiskEngine` 和 `GateMonitor` 的一票否决。
- 训练报告必须声明单一 `training_symbol`、`bar_interval_ms` 和
  `online_bar_source=closed_ohlcv`。
- Public WS 同时接收 ticker 和已确认 kline；ticker 驱动策略与执行，只有
  `confirm=true`、周期匹配、币种匹配的 kline 才进入 Integrator 特征引擎。
- 生产启动从训练 CSV 补齐最近 `feature_window_ticks` 个已收盘 bar；replay
  已自带历史序列，禁止二次 bootstrap。特征过期超过两个 bar 周期时模型自动禁用。
...
- 严格反事实窗口按时间切成 train/holdout；train 只负责选权重，holdout 只负责验收。
- train/holdout 任一低于样本门槛时冻结更新，不允许全窗口原位选优或 factor-IC 旁路。
- Sharpe-like 目标使用 `objective_min_risk_scale` 限制风险分母，避免零回撤窗口分数爆炸。
...
- `integrator.*`：CatBoost 时序切分、候选版本、特征契约及分级接管
- `gate.*`：活跃度门槛
...
### 12.2 集成测试
- Replay：固定样本回放一致性
- Candidate replay：训练、回放、注册和 live 的 `model_version`/模型哈希一致
- Replay 状态隔离：每个 symbol/segment 使用全新 WAL 目录，禁止恢复 live、前段或
  重试残留状态
- Replay 终态核算：数据耗尽后撤销全部非终态单（含 SL/TP）、以最终 mark 强制
  taker 减仓至空仓；
  终态账户净收益作为经济分子，当前 boot 全部成交作为分母，任一证据缺失即失败
- Feature contract：训练、回放和 live 的 symbol/bar interval/closed-OHLCV
  语义一致；ticker、未收盘 bar、错币种和过期特征均不能触发推理
- 经济曲线：forward label 不得作为逐 bar 收益重复复利，split 边界必须 purge/embargo
- 成交账本：`fill_id` 去重、冲突重复失败、净收益可由成交重建
