...
- `src/market`：WS/REST Adapter + RuleCache
- `src/universe`：UniverseSelector（智能/AI筛币）
- `src/feature`：FeatureEngine
- `src/regime`：RegimeEngine
- `src/strategy`：StrategyEngine + plugins
- `src/portfolio`：PortfolioEngine
- `src/risk`：RiskEngine
...
### 9.3 学习触发门槛
- `decision_epochs >= min_decision_epochs`
- `fills >= min_fills_for_update`
- `improvement_ratio >= improvement_threshold`
...
- 组合/学习：
  ...
  - `LEARN_SKIPPED_INSUFFICIENT_DATA`
- OMS：
