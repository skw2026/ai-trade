...
3. 提交日志时至少包含以下区段：
   ...
   - 说明：若使用 `--since` 截窗验收且窗口不含启动段，允许用 `SELF_EVOLUTION_ACTION`
     或 `RUNTIME_STATUS ... evolution={enabled=true,...}` 证明进化链路已初始化；
     对 `policy-flat` 主导且全空窗的静默进化，不再强制要求截窗内出现动作日志。
4. 我按“阶段闸门表”给结论：
   - `PASS`：可进入下一阶段
...
- [ ] 空仓且无净仓位在途订单时，远端快照短暂不可用应输出 `OMS_RECONCILE_DEGRADED_FLAT_IDLE` 并最终看到 `OMS_RECONCILE_ANOMALY_HALT_EXIT`
...
Demo 孵化晋级必须读取 `latest_demo_incubation_report.json`，不得以单次 Closed Loop
成功或单次回测盈利替代。最低要求由 `config/demo_incubation_policy.json` 冻结；
在结果达到 `ELIGIBLE_FOR_MANUAL_LIVE_TEST_REVIEW` 前，主网实盘连接必须保持禁用。
...
- 运行验收脚本中的 Gate 失败率门槛：
  - S5：当 Gate 窗口数 `>=10` 且 `gate_check_fail_ratio > 0.90` 直接 FAIL；
  - S5：当 Gate 窗口数 `>=8` 且 `gate_check_fail_ratio > 0.85` 标黄（PASS_WITH_ACTIONS）。
- S5 强闭环附加约束（阻断项）：
  - [ ] 至少出现一次 `GATE_CHECK_PASSED`（避免“全窗口失败”被误判可用）。
  - [ ] 运行窗口起点应为平仓状态（`|first_notional| <= 50 USD`），否则本轮 S5 结果不可用于策略有效性评估。
  - [ ] `strategy_mix_nonzero_window_count >= 1`（至少出现一个非零策略窗口），否则按 FAIL 处理。
  - [ ] 若 `gate.allow_policy_flat_windows=true` 且窗口以 `policy-flat` 为主，
        可豁免“零信号/零成交”阻断项，但必须输出 warning，不能静默视为正常活跃交易。
  - [ ] `runtime_assess` 必须显式区分：
        `protection_status`（坏 bucket 主动空仓是否正确）与
        `execution_status`（真实执行质量是否完成验证）。
        `execution_status=NOT_EVALUATED` 不能视为“执行通过”。
  - [ ] `runtime_assess` 必须输出 `market_context_status` 与 `account_sync_status`，
        用于区分“当前窗口没有 TREND 样本，因此 execution 暂未验证”与
        “账户权益口径存在外部噪声，需先排查统计口径”。
  - [ ] 若 `TREND` 桶窗口达到最小门槛，则必须形成策略参与或执行样本，
        防止系统退化为“所有行情都不交易”。

#### S5 补充：长期缺少 live TREND 样本时的替代放行规则

用途：
- 避免项目因为行情长期 `RANGE_ONLY` 而无限停滞；
- 同时避免把 replay 误写成 live execution 真证据。

证据分层：
- [ ] 第一层 `runtime protection`：
      `runtime_health_status=PASS/PASS_WITH_ACTIONS`，且 `protection_status=PASS`；
      无 `CRITICAL`、WS 异常、不可恢复对账异常、不可恢复 halt。
- [ ] 第二层 `replay trend execution`：
      `replay_readiness_status=PASS`，且优先要求
      `sections.replay_validation.aggregate_validation.coverage_strength_status=ROBUST`。
- [ ] 第三层 `live trend execution`：
      必须出现 `market_context_status=TREND_PRESENT`，并形成 live 策略参与或成交样本。

放行等级：
- [ ] `L1 替代放行`：
      当第一层和第二层都满足，且 `trend_validation_status=PASS`、`promotion_readiness_status=PASS` 时，
      允许继续闭环验证、继续 shadow/canary、继续保守部署。
- [ ] `L2 真放行`：
      只有第三层也满足，才允许把“live 趋势 execution 已验证”当成成立结论，
      并作为更激进上线/接管的前提。

禁止事项：
- [ ] 若长期没有 `live TREND`，不得继续围绕趋势执行参数做高频微调。
- [ ] `replay_readiness_status=PASS` 不能单独替代 `live trend execution` 完成声明。
- [ ] `execution_status=NOT_EVALUATED` 只能解释为“当前窗口未完成 live execution 验证”，
      不能解释为“execution 已通过”。
...
- [ ] Case O: 保护单启用且开仓成功（SL 及时挂出，TP可选；S5 观测项需看到 `PROTECTION_REFRESH`）
- [ ] Case P: SL 挂单失败（触发 `EXEC_PROTECTIVE_ORDER_MISSING` + 进入 `REDUCE_ONLY`/冻结；S5 必检项）
- [ ] Case Q: TP 挂单失败（记录 `EXEC_TP_ATTACH_FAILED`，持仓继续但可审计；S5 观测项）
...
- [ ] 报告包含代际统计：`generation_summaries`（best/mean objective 可追踪）
