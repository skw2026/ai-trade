...
脚本现在会按 `available_segments` 从长到短自动回放，直到：
- 达到 `min_execution_active_runs`
- 达到 `min_execution_pass_runs`
- 达到 `min_total_fills`

满足覆盖门槛后会提前停止，不再无意义地继续跑剩余片段。可通过 `selection.coverage_targets_met` 和 `selection.segments_ran` 直接判断这次 replay 是否已经拿到足够趋势 execution 样本。
