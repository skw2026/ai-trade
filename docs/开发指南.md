...
```bash
./build/trade_bot \
  --config=config/bybit.replay.assess.yaml \
  --exchange=bybit \
...
python3 tools/run_replay_validation.py \
  ...
  --base_config=config/bybit.replay.assess.yaml \
...
`config/bybit.replay.assess.yaml` 当前默认采用“接近 live/S5 的 execution 与 strategy 主参数”，
但仍保留 replay 特有的短窗口适配（如更低的 `adaptive_fee_gate_min_samples`），
这样 replay 结果更接近线上口径，同时不至于因为片段太短而完全失真。
