...
## 0. 术语统一（文档/代码/注释一致口径）
- 净名义敞口（`net notional exposure`）：
  - 定义：`qty * mark_price`，带方向（多为正，空为负）
  - 对应字段：`suggested_notional_usd`、`target_notional_usd`、`adjusted_notional_usd`
- 总名义敞口（`gross notional exposure`）：
  - 定义：`sum(abs(symbol_notional))`
  - 用途：账户级风险预算与多币种总暴露控制
- 只减仓（`reduce-only`）：
  - 定义：只允许持仓绝对值向 0 收敛，不允许新增敞口或反手开仓
  - 对应字段：`OrderIntent.reduce_only=true`
- 最小名义金额（`min_notional`）：
  - 定义：交易所规则要求的最小可成交名义金额
  - 约束：执行前必须前置过滤；被过滤记录 `EXEC_FILTER_IGNORE`
...
- symbol：BTC/ETH/SOL 永续标的（规范化，如 BTCUSDT；永续类型由合约元数据标识）
- version_set：
  - config_version
...
struct EventHeader {
    ...
    struct VersionSet {
        ...
        std::string strategy_bundle_version;
        std::string regime_version;
        std::string git_commit; // 可选，建议 Gate/Shadow 必填
    } version;
...
- regime_bucket：TREND/RANGE/EXTREME（由上游映射：UPTREND/DOWNTREND→TREND）
- confidence：0..1
...
### 2.4 Signal（策略输出）
强制口径：
- suggested_notional_usd（唯一敞口口径，带符号净敞口）
字段要点：
- signal_type：TARGET_POSITION / RISK_ONLY
- direction：-1/0/1（必须与 suggested_notional_usd 的符号一致）
...
struct Signal {
    ...
    double suggested_notional_usd; // 核心输出（signed）
    int direction;                 // 1, -1, 0；需与notional符号一致
...
- weight_constraints（建议）：sum_weight、is_normalized、has_negative_weight
- reason_codes：PORT_* +（继承部分STR/REG）

### 2.6 RiskAction / RiskAdjustedPosition（风控输出）
RiskAction：
- action_type：DOWNGRADE/REDUCE/COOLDOWN/FUSE/REDUCE_ONLY/FREEZE_SYMBOL/UNFREEZE_SYMBOL/DUST_LOCKED/DUST_UNLOCKED
- severity：1..5
...
struct RiskAdjustedPosition {
    ...
    RiskMode risk_mode;
    std::vector<std::string> reason_codes; // RISK_*
...
- parent_order_id（可选；保护单可关联开仓主单）
- side、type、qty、price、tif
- purpose：ENTRY/TP/SL/REDUCE
- reduce_only（保护单必须为 true）
- state：SENT/ACK/REJECTED/CANCELLED/FILLED/PARTIAL
  - 允许转移：`SENT->ACK|REJECTED|CANCELLED`，`ACK->PARTIAL|FILLED|CANCELLED|REJECTED`，`PARTIAL->PARTIAL|FILLED|CANCELLED|REJECTED`
FillEvent：
- fill_qty、fill_price、fee、slippage_bps（可计算）

执行语义约束（必须）：
- 反向调仓按“先平后开”执行：先发 `purpose=REDUCE + reduce_only=true` 平掉旧方向，再由后续信号发 `purpose=ENTRY` 开新方向。
- 非减仓目的禁止设置 `reduce_only=true`，避免出现“同边 reduce-only”拒单。

## 3. 幂等与时序规则
- client_order_id 生成规则：{symbol}-{ts_local_ms}-{seq}-{intent_hash}
- 重试必须使用同一个 client_order_id
- client_order_id 必须在首次生成后持久化（随 OrderIntent 落地）；重试路径禁止重算
- 回放时以 seq 排序重建处理顺序
