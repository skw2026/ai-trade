...
- 实现约束：账户级总名义敞口（gross）上限裁剪已在决策流水线生效
- 阶段2：Bandit调权（受控、可回滚）

8) RiskEngine（Hard）
- 风控状态机：NORMAL/DEGRADED/COOLDOWN/FUSE/REDUCE_ONLY
- 应用硬规则：回撤阈值、强平距离、杠杆白名单、逐仓强制、断连只减仓
- 强平距离口径：账户状态按远端持仓同步的 `liquidation_price` 计算“加权P95强平距离”
- 运行时一致性：除启动同步外，按 `system.remote_risk_refresh_interval_ticks` 周期刷新远端 `liqPrice/markPrice`
- 输出：RiskAdjustedPosition + RiskActions

9) ExecutionEngine
- 目标仓位逼近（Rebalance）
- 下单前最小名义金额前置过滤（避免“买不起/卖不动”拒单循环）
...
- 计算 drawdown、净名义敞口、总名义敞口、liq_distance_p95
- 对账：与交易所定时比对差异
...
12) Stage3 Research Pipeline（因子-模型-风控闭环前两级，规划）
- Miner（离线）：符号回归/遗传规划，输出 `factor_set_version + factor_expressions`
- Integrator（离线训练 + 在线影子）：CatBoost 输出 `model_version + score/p_up/p_down`
- ModelRegistry（版本治理）：模型签名、schema 校验、回滚与灰度发布
- 约束：Research Pipeline 不能直接下单；必须经 Strategy/Portfolio/Risk 主链路与 Gate 放行
...
Market/WAL/Archive
→ Miner（因子挖掘）
→ FactorStore（versioned）
→ Integrator Trainer（Rolling/TimeSeriesSplit）
→ ModelRegistry（versioned）
→ Online Shadow Inference（只写审计，不直接下单）
