...
- 新增执行质量硬保护参数：
  ...
  - 执行质量守卫同时维护全局与 symbol 级状态：如果某个币对的完整净仓位成交（entry/reduce/TP/SL）成本/净质量持续劣化，会只对该币对叠加 `quality_guard_required_edge_penalty_bps`，避免 BNB/SOL 这类单币低质量样本被全局均值稀释，也避免“一刀切”降低所有币对流量。
