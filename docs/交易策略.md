...
- probe 仓位是执行质量取样仓，不等同于普通主策略仓位；可用 `candidate_probe_reduce_max_adverse_bps` 设置更早的专用 adverse cut，防止小额试错仓被普通 `strategy_reduce_cost_guard` 拖到更深亏损才退出。
阶段2增强：
...
- 执行质量守卫同时维护全局与 symbol 级状态：如果某个币对的完整净仓位成交（entry/reduce/TP/SL）成本/净质量持续劣化，会只对该币对叠加 `quality_guard_required_edge_penalty_bps`，避免 BNB/SOL 这类单币低质量样本被全局均值稀释，也避免“一刀切”降低所有币对流量。低质量记忆未清零前，普通主链开仓会被 `symbol_quality_quarantine` 隔离；硬冷却结束后仅允许小额 `TREND_CANDIDATE` probe 恢复验证，正质量样本把记忆衰减清零后才恢复主链开仓。
  ...
  - 普通策略 reduce 增加成本守门：若预计出场后净 bps 仍低于 `strategy_reduce_min_net_bps`，会拦截 `STRATEGY_REDUCE_COST_GUARD_BLOCKED`，避免“没有足够净收益空间就平仓”；风控强制 reduce-only、SL/TP、EXTREME、最大亏损止损和最大持仓超时会绕过该守门，防止为了覆盖成本而拒绝必要止损。
