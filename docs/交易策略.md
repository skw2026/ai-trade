...
- `REGIME_CHANGE` 与 `RUNTIME_STATUS.regime_current` 输出 `trend_threshold_ratio / volatility_threshold_ratio / trend_candidate / warmup_trend_candidate`；报告汇总 `TREND_CANDIDATE`、warmup candidate、candidate 窗口数和最大趋势门槛比率，用于判断长期无 live TREND 是门槛过高、确认周期过长、Universe 轮换过快，还是确实没有可交易趋势。
