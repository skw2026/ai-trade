...
- 触发过 symbol 级执行质量守卫的币对会进入“硬冷却 + 软记忆”：硬冷却期间探针直接跳过并记录 `QUALITY_GUARD_MEMORY`；软记忆未通过后续正样本衰减前，探针会按触发次数降名义值，且不允许 fee override 或 near-miss maker 放行。
阶段2增强：
...
- 执行质量守卫同时维护全局与 symbol 级状态：如果某个币对的完整净仓位成交（entry/reduce/TP/SL）成本/净质量持续劣化，会只对该币对叠加 `quality_guard_required_edge_penalty_bps`，避免 BNB/SOL 这类单币低质量样本被全局均值稀释，也避免“一刀切”降低所有币对流量。低质量记忆未清零前，普通主链开仓会被 `symbol_quality_quarantine` 隔离；硬冷却结束后仅允许小额 `TREND_CANDIDATE` probe 恢复验证，正质量样本把记忆衰减清零后才恢复主链开仓。
  - symbol 级守卫使用约 `100 USD` / `0.5x signal_notional` 的低 notional 样本门槛，并将单币最小成交评估门槛封顶到 6 fills，专门覆盖 S5 小额 probe/往返磨损；S5 默认 4 fills 且 1 个坏窗口即可进入单币冷却，避免同一币对在完整扣费后净亏时继续拿真仓探路。明显劣化窗口可单窗口触发，一旦某币对守卫激活，`TREND_CANDIDATE` 的 fee override 不再绕过成本门。
