...
self_evolution:
  ...
  objective_gamma_notional_churn: 0.005 # 目标函数：名义敞口抖动惩罚系数（USD）
...
4. **校验**: 
  ...
  - 检查权重约束（`0<=w_i<=max_single_strategy_weight`、权重和=1）。
   ...
   - 检查 Integrator 接管参数：`mode` 属于 `off/shadow/canary/active`；`canary_notional_ratio/canary_confidence_threshold/active_confidence_threshold` 均在 `[0,1]`，且 `canary_min_notional_usd >= 0`。
