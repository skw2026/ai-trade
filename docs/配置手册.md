...
- `execution_quality_guard_symbol_active_count_max / execution_quality_guard_symbol_active_count_latest`：运行态日志中的 symbol 级执行质量守卫覆盖数；`max` 表示本轮曾经覆盖多少币对，`latest` 表示报告结束时仍有多少币对在惩罚/冷却。只有 `latest>0`、`ENTER` 未被 `EXIT` 覆盖，或出现 `REINFORCE` 时才应视为需要整改的执行退化；若只是 `ENTER` 后正常 `EXIT`，更接近短暂自愈事件。
...
4. **校验**: 
  ...
  - 检查权重约束（`0<=w_i<=max_single_strategy_weight`、权重和=1）。
   ...
   - `TREND_CANDIDATE_PROBE_SKIPPED` 会输出 `ratio_low/cooldown/exposure/pending_orders/existing_intent/window_limit` 等原因；若 `TREND_CANDIDATE` 较多但探针样本不足，应优先看这些 skip reason，而不是继续盲目等待 12/24h。
