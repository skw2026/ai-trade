...
- `integrator_nan_feature_skip_count`：`INTEGRATOR_SKIP: NaN feature detected` 次数；大于 0 时需复核 miner/live 特征对齐。日志会输出 `symbol/regime/bucket/feature_index/feature_name/raw_value/model_version`，报告会汇总 `integrator_nan_feature_skip_by_feature`、`integrator_nan_feature_skip_by_symbol` 和最多 5 条 `integrator_nan_feature_skip_examples`，用于定位具体异常特征。
