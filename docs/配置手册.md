...
- `CLOSED_LOOP_REPLAY_VALIDATION_DEFAULT_SYMBOLS`（当 `CLOSED_LOOP_REPLAY_VALIDATION_SYMBOLS` 为空时的默认 replay 多币对，默认 `SOLUSDT,ETHUSDT,BTCUSDT,XRPUSDT,BNBUSDT`，用于避免 full/train/data 跑完后只留下单一 replay 结果，并让 S5 canary 优先验证当前可交易白名单）
- `CLOSED_LOOP_REPLAY_VALIDATION_SYMBOL`（`replay_validation` 单目标币对；默认跟随 `SYMBOL`）
- `CLOSED_LOOP_REPLAY_VALIDATION_SYMBOLS`（`replay_validation` 多目标币对，逗号分隔；设置后会在同一轮中按币对分别 replay，并输出 `symbols/symbol_reports/per_symbol_*` 覆盖结果；手动和定时 closed-loop workflow 默认覆盖 `SOLUSDT,ETHUSDT,BTCUSDT,XRPUSDT,BNBUSDT`。若报告出现 `replay_symbol_alignment` 告警，应优先使用 `sections.replay_symbol_alignment.recommended_replay_symbols_csv` 作为下一轮目标，避免 replay 只验证了部分币对）
- `CLOSED_LOOP_REPLAY_VALIDATION_SOURCE_SYMBOL`（`feature_csv` 对应的源行情币对；默认跟随 `SYMBOL`。若 `SOURCE_SYMBOL` 与 `SYMBOLS` 不一致，报告会告警该 replay 只能验证执行链路/配置覆盖，不能替代目标币对真实历史行情验证）
...
- `CLOSED_LOOP_REPLAY_VALIDATION_MIN_TRADABLE_SYMBOLS`（多币对 replay 至少需要多少币对满足覆盖与净收益条件，默认 `1`，与 S5 `SOLUSDT` 单币 canary 对齐；覆盖充分但经济性为负的币对进入 `quarantined_symbols`，覆盖不足的非 source 币对只作为观察项，不再拖垮已通过的主链白名单）
...
- `CLOSED_LOOP_MAX_AUC_STDEV`（integrator AUC 稳定性治理阈值，默认 `0.09`；用于避免 `0.08` 附近的统计毛刺反复阻断注册）
  ...
  `SOLUSDT`，`ETHUSDT/BTCUSDT/XRPUSDT/BNBUSDT` 继续 replay 和离线观察，直到 median/positive ratio 恢复。
