...
- `CLOSED_LOOP_REPLAY_VALIDATION_MAX_SEGMENTS`（`replay_validation` 最大片段数，默认 `8`）
- `CLOSED_LOOP_REPLAY_VALIDATION_MIN_SEGMENT_BARS`（`replay_validation` 单片段最少 bars，默认 `60`）
