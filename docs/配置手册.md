...
- `execution_quality_guard_symbol_active_count_max / execution_quality_guard_symbol_state_count_max`：运行态日志中的 symbol 级执行质量守卫覆盖数；active 覆盖包含仍在冷却期的币对。symbol 级质量使用闭合成交（reduce/TP/SL 或产生 realized PnL 的成交）的 realized-net，并使用约 `100 USD` / `0.5x signal_notional` 的低 notional 门槛与最多 6 fills 的单币评估门槛，用于识别低质量币对是否被单独惩罚，而不是被全局均值掩盖；entry-only 成交只参与 fee 异常判断，不再把天然 `-fee` 当作净亏。
