...
- `execution_symbol_quality_guard_enter_count / execution_symbol_quality_guard_exit_count`：symbol 级执行质量守卫进出场事件计数。
- `execution_quality_guard_symbol_active_count_max / execution_quality_guard_symbol_state_count_max`：运行态日志中的 symbol 级执行质量守卫覆盖数；symbol 级质量使用完整净仓位成交（entry/reduce/TP/SL）的 realized-net，并使用约 `100 USD` / `0.5x signal_notional` 的低 notional 门槛与最多 6 fills 的单币评估门槛，用于识别低质量币对是否被单独惩罚，而不是被全局均值掩盖。
