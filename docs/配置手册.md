...
- 成本门细分：`entry_gate.observed_filtered_ratio`、`entry_gate.observed_near_miss_ratio`、`execution_window.filtered_cost_near_miss_ratio`
...
4. **校验**: 
  ...
  - 检查权重约束（`0<=w_i<=max_single_strategy_weight`、权重和=1）。
   ...
   - 检查执行成本参数：`execution.entry_fee_bps/exit_fee_bps/expected_slippage_bps/min_expected_edge_bps/required_edge_cap_bps/entry_gate_near_miss_tolerance_bps/maker_price_offset_bps >= 0`。
