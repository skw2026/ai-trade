...
# - 当 `use_counterfactual_search=true` 且 `enable_factor_ic_adaptive_weights=true` 同时开启时，
#   若反事实候选未达最小改进门槛，会自动回退到 factor-IC 调权，避免“有开关但无动作”。
...
4. **校验**: 
  ...
  - 检查自进化虚拟PnL参数：`self_evolution.counterfactual_min_improvement_usd >= 0`、`self_evolution.counterfactual_improvement_decay_per_filtered_signal_usd >= 0`、`self_evolution.virtual_cost_bps >= 0`。
