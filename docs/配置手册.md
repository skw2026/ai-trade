...
- `CLOSED_LOOP_INTEGRATOR_FEATURE_CLIP_QUANTILE`（R2 特征稳健裁剪分位数，默认 `0.001`；训练报告会写入 `feature_transform`，线上 Integrator 推理会按同一边界裁剪）
...
- `integrator.*` 用于 CatBoost 训练与影子推理（Integrator）。
  ...
  - 特征稳健裁剪支持：
    ...
    - 报告 `feature_transform.clip_bounds` 会记录每个特征的训练分布裁剪边界。
    - C++ `IntegratorShadow` 会读取该边界并在 live 推理前按同一口径裁剪，降低 `miner_*` 原始价格尺度或异常值导致的 train/live 分布错位风险。
