...
- `CLOSED_LOOP_REPLAY_VALIDATION_SYMBOLS`（`replay_validation` 多目标币对，逗号分隔；设置后会在同一轮中按币对分别 replay，并输出 `symbols/symbol_reports/per_symbol_*` 覆盖结果；当 live TREND/TREND_CANDIDATE 长期出现在非主币对时，优先设置为这些币对，例如 `SOLUSDT,XRPUSDT`）
