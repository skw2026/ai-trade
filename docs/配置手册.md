...
- `CLOSED_LOOP_WALKFORWARD_MIN_AVG_SPLIT_RETURN`（walk-forward 经济性强门禁：`avg_split_return` 下限，默认 `0.0`）
- `CLOSED_LOOP_WALKFORWARD_MIN_ENABLED_AVG_SPLIT_RETURN`（walk-forward 经济性强门禁：`enabled_avg_split_return` 下限，默认 `0.0`）
- `CLOSED_LOOP_WALKFORWARD_MIN_TRADED_AVG_SPLIT_RETURN`（walk-forward 经济性强门禁：`traded_avg_split_return` 下限，默认 `0.0`）
...
- `CLOSED_LOOP_REPLAY_VALIDATION_REAL_MARKET_FEATURES`（是否在 replay 前为 `CLOSED_LOOP_REPLAY_VALIDATION_SYMBOLS` 逐币对生成各自 feature store，默认 `true`；成功后 `replay_validation.real_market_replay=true`，否则会退化为源 feature store 覆盖验证并保留 warning）
...
- `CLOSED_LOOP_REPLAY_VALIDATION_REFRESH_CORPUS`（是否忽略已有 corpus manifest 并按当前 `feature_store` 重建，默认 `false`）
