...
- `CLOSED_LOOP_REPLAY_VALIDATION_SYMBOLS`（`replay_validation` 多目标币对，逗号分隔；设置后会在同一轮中按币对分别 replay，并输出 `symbols/symbol_reports/per_symbol_*` 覆盖结果；手动 closed-loop workflow 默认覆盖 `BTCUSDT,ETHUSDT,SOLUSDT,XRPUSDT,BNBUSDT`。若报告出现 `replay_symbol_alignment` 告警，应优先把 live TREND/TREND_CANDIDATE 符号补进该列表，避免 replay 只验证了部分币对）
