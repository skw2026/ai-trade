...
- `CLOSED_LOOP_REPLAY_VALIDATION_DEFAULT_SYMBOLS`（当 `CLOSED_LOOP_REPLAY_VALIDATION_SYMBOLS` 为空时的默认 replay 多币对，默认 `BTCUSDT,ETHUSDT,SOLUSDT,XRPUSDT,BNBUSDT`，用于避免 full/train/data 跑完后只留下 `BTCUSDT` replay 结果）
- `CLOSED_LOOP_REPLAY_VALIDATION_SYMBOL`（`replay_validation` 单目标币对；默认跟随 `SYMBOL`）
- `CLOSED_LOOP_REPLAY_VALIDATION_SYMBOLS`（`replay_validation` 多目标币对，逗号分隔；设置后会在同一轮中按币对分别 replay，并输出 `symbols/symbol_reports/per_symbol_*` 覆盖结果；手动和定时 closed-loop workflow 默认覆盖 `BTCUSDT,ETHUSDT,SOLUSDT,XRPUSDT,BNBUSDT`。若报告出现 `replay_symbol_alignment` 告警，应优先使用 `sections.replay_symbol_alignment.recommended_replay_symbols_csv` 作为下一轮目标，避免 replay 只验证了部分币对）
...
- `CLOSED_LOOP_REPLAY_VALIDATION_MIN_TRADABLE_SYMBOLS`（多币对 replay 至少需要多少币对满足覆盖与净收益条件，默认 `3`；覆盖充分但经济性为负的币对进入 `quarantined_symbols`，不再拖垮已通过的主链白名单）
- `CLOSED_LOOP_WALKFORWARD_FOCUS_BUCKET`（registry walk-forward 主链口径，默认 `trend`；当该桶满足 `CLOSED_LOOP_TREND_VALIDATION_*` 门槛时，RANGE/EXTREME 拖累导致的全局 split 负收益降级为告警）
- `CLOSED_LOOP_MAX_AUC_STDEV`（integrator AUC 稳定性治理阈值，默认 `0.09`；用于避免 `0.08` 附近的统计毛刺反复阻断注册）
  ...
  `tradable_symbols` 放入 `config/bybit.demo.s5.yaml` 的 `candidate_symbols`；被隔离币对仍可继续 replay
  和离线观察，但不会因为单币种负经济性阻断 BTC/ETH/SOL 这类已通过币对的主链验证。
