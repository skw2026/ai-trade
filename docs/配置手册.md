...
- `execution_quality_guard_symbol_active_count_max / execution_quality_guard_symbol_state_count_max`：运行态日志中的 symbol 级执行质量守卫覆盖数；symbol 级质量使用完整净仓位成交（entry/reduce/TP/SL）的 realized-net，并降低小额 probe 的样本门槛，用于识别低质量币对是否被单独惩罚，而不是被全局均值掩盖。
