...
self_evolution:
  ...
  counterfactual_min_fill_count_for_update: 0 # 反事实更新最小成交样本门槛（0=关闭）
...
# - 当 `use_counterfactual_search=true` 时，更新路径严格走反事实候选；
#   若反事实候选未达到改进/样本门槛，会跳过本轮更新（不再自动回退到 factor-IC 调权）。
...
- `SCHEDULER_ACTION`（scheduler 每轮执行动作：`train|assess|full`，默认 `full`）
...
- `runtime.warn_reasons` 中，S5 当 `self_evolution_action_count >= 30`、`self_evolution_learnability_pass_count >= 10` 且
  `self_evolution_counterfactual_update_count + self_evolution_factor_ic_action_count = 0` 时标黄，
  用于识别“学习长期空转（只评估不更新）”。
...
4. **校验**: 
  ...
  - 检查权重约束（`0<=w_i<=max_single_strategy_weight`、权重和=1）。
   ...
   - 检查成本门扩展参数：`adaptive_fee_gate_trigger_ratio ∈ [0,1]`、`adaptive_fee_gate_max_relax_bps >= 0`、`maker_edge_relax_bps >= 0`、`cost_filter_cooldown_trigger_count` 与 `cost_filter_cooldown_ticks` 需同时>0或同时为0。
