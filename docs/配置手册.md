...
# - 目标函数已统一为“权益归一化后的 bps 量纲”：
#   pnl_bps / drawdown_bps / churn_bps，避免账户规模变化导致惩罚权重失真。
...
4. **校验**: 
  ...
  - 检查权重约束（`0<=w_i<=max_single_strategy_weight`、权重和=1）。
   ...
   - 检查 Integrator 接管参数：`mode` 属于 `off/shadow/canary/active`；`canary_notional_ratio/canary_confidence_threshold/active_confidence_threshold/active_partial_notional_ratio/active_full_notional_confidence_threshold` 均在 `[0,1]`，且 `canary_min_notional_usd >= 0`。
