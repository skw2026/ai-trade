...
execution:
  ...
  quality_guard_required_edge_penalty_bps: 1.5 # 守卫激活时附加到 required_edge 的惩罚；symbol级会叠加到对应币对并阻止 probe fee override
...
- `execution_quality_guard_symbol_active_count_max / execution_quality_guard_symbol_state_count_max`：运行态日志中的 symbol 级执行质量守卫覆盖数；active 覆盖包含仍在冷却期的币对。symbol 级质量使用完整净仓位成交（entry/reduce/TP/SL）的 realized-net，并使用约 `100 USD` / `0.5x signal_notional` 的低 notional 门槛与最多 6 fills 的单币评估门槛，用于识别低质量币对是否被单独惩罚，而不是被全局均值掩盖。
