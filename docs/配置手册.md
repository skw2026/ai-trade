...
execution:
  ...
  quality_guard_required_edge_penalty_bps: 1.5 # 守卫激活时附加到 required_edge 的惩罚；symbol级会叠加到对应币对
...
- `execution_quality_guard_symbol_active_count_max / execution_quality_guard_symbol_state_count_max`：运行态日志中的 symbol 级执行质量守卫覆盖数；用于识别低质量币对是否被单独惩罚，而不是被全局均值掩盖。
