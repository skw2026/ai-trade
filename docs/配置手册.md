...
4. **校验**: 
  ...
  - 检查权重约束（`0<=w_i<=max_single_strategy_weight`、权重和=1）。
   ...
   - `TREND_CANDIDATE_PROBE_SKIPPED` 会输出 `ratio_low/cooldown/quality_guard_memory/exposure/pending_orders/existing_intent/window_limit` 等原因；若 `TREND_CANDIDATE` 较多但探针样本不足，应优先看这些 skip reason，而不是继续盲目等待 12/24h。
