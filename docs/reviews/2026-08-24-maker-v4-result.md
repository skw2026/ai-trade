...
不可变 Research `#1095` 在 commit `1ee34b6` 上技术成功。v5 产生 79 笔 oracle 交易，正 stress split 比例为 1.0，base/stress LCB 为 `5.1226/2.8101 bps`，但仍低于 100 笔且 boundary pass ratio 为 0。最终决策为 `STOP_MAKER_EXECUTION_FAMILY`；没有 forward 等待、模型训练、Demo 或 live 权限。

`common_row_count` 从 v4 的 237965 变为 237126，来源是运行时整段 capture 窗口变化；绝对 primary/boundary split 仍继承 v2。该差异不改变 `79 < 100` 与 boundary 为 0 的决定性结论，但也暴露出跨代实验应显式绑定重叠冻结区间字段哈希。后续新机制把该绑定列为硬前置。

下一阶段见 [v5 关闭复盘与残差机制预注册](2026-08-24-maker-v5-closure-roundtable.md)。
