...
结案接续（2026-09-23）：同一gate一次纠正绑定已通过。新提交`2a4f464`的
CD `35757951133`学习步骤和实际部署均成功，最终学习/部署后收据通过，gate READY。
以下记录是原失败事实，不改为PASS；完整结果见[阶段结案](2026-09-22-offline-learning-loop-result.md)。
