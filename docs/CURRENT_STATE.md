...
- 仓库取消 `closed-loop.yml` 的每日 research cron 和 `option-lifecycle-v4.yml` 的每小时经济重算 cron；保留显式研究请求、手动复算和必要的发布后验证。取消定时触发不撤销已经在途的任务。
- 不改 ECS 运行策略、账户模式、仓位、订单、资金或交易 release；watchdog、scheduler assess、行情采集与原有风控不因本轮研究关闭而停止。
- 本地修改不等于远端生效。默认分支发布、在途任务检查、只读运行证据与五项验收结果统一记录在[三步收口交付结果](reviews/2026-09-20-three-step-closeout-result.md)。未取得的远端证据标为证据不足。
