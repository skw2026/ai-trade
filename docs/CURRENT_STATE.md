...
最新接续（2026-09-23）：用户选择**独立安全撤回**。本批本地工程通过：最终本机119/119、Linux119/119，原模型在第二个负窗/第1200根bar撤回，权重保持0.55，后336次观测不解锁；应用/发送/持久化/重启、多桶和重报价边界已覆盖。首轮Linux临时替身受/tmp noexec影响先失败，确认根因、固定CTest构建内临时目录后原命令唯一复验通过；旧失败和四份HALTED均保留。**当前准备main/CI/CD交付，尚未声称远端通过或线上新开关已启用。** config profile和候选资格不改。见[阶段结果](reviews/2026-09-23-independent-safety-withdrawal-result.md)、[本地证据](reviews/2026-09-23-independent-safety-withdrawal.local-evidence.json)及[行为合同](EVOLUTION_SAFETY.md)。以下旧“待选择”是历史时点，不覆盖本次明确选择。
