...
HOLDOUT_CONSUMPTION_LEDGER_PATH="${CLOSED_LOOP_HOLDOUT_CONSUMPTION_LEDGER_PATH:-${AI_TRADE_DATA_DIR:-./data}/models/final_holdout_consumption.jsonl}"
...
MICROSTRUCTURE_CAPTURE_ROOT="${CLOSED_LOOP_MICROSTRUCTURE_CAPTURE_ROOT:-${AI_TRADE_DATA_DIR:-./data}/research/microstructure}"
...
LIQUIDATION_CAPTURE_ROOT="${CLOSED_LOOP_LIQUIDATION_CAPTURE_ROOT:-${AI_TRADE_DATA_DIR:-./data}/research/bybit_sol_liquidations}"
...
MAKER_OPPORTUNITY_BASELINE_AUDIT_MANIFEST="${CLOSED_LOOP_MAKER_OPPORTUNITY_BASELINE_AUDIT_MANIFEST:-${AI_TRADE_DATA_DIR:-./data}/research/maker_opportunity_frozen_audit.json}"
...
OPTION_VRP_SEQUENTIAL_CONFIG="${CLOSED_LOOP_OPTION_VRP_SEQUENTIAL_CONFIG:-config/option_variance_risk_premium_sequential_payoff_v2.json}"
OPTION_VRP_SEQUENTIAL_MANIFEST="${CLOSED_LOOP_OPTION_VRP_SEQUENTIAL_MANIFEST:-config/option_variance_risk_premium_sequential_payoff_manifest_v2.json}"
OPTION_VRP_V2_CAPTURE_ROOT="${CLOSED_LOOP_OPTION_VRP_V2_CAPTURE_ROOT:-${AI_TRADE_DATA_DIR:-./data}/research/bybit_btc_option_vrp_v2}"
...
DECISION_EVIDENCE_LEDGER_PATH="${CLOSED_LOOP_DECISION_EVIDENCE_LEDGER:-${AI_TRADE_DATA_DIR:-./data}/research/experiment_budget_ledger.jsonl}"
...
CROSS_VENUE_FUNDING_HISTORY_PATH="${RUN_DIR}/cross_venue_funding_history.csv"
CROSS_VENUE_FUNDING_DATA_REPORT_PATH="${RUN_DIR}/cross_venue_funding_data_report.json"
...
OPTION_VRP_SEQUENTIAL_REPORT_PATH="${RUN_DIR}/option_variance_risk_premium_sequential_payoff.json"
...
ACTIVE_ALPHA_ROUTE=""
ACTIVE_MODEL_PATH="${AI_TRADE_DATA_DIR:-./data}/models/integrator_latest.cbm"
ACTIVE_REPORT_PATH="${AI_TRADE_DATA_DIR:-./data}/research/integrator_report.json"
ACTIVE_MINER_REPORT_PATH="${AI_TRADE_DATA_DIR:-./data}/research/miner_report.json"
ACTIVE_META_PATH="${AI_TRADE_DATA_DIR:-./data}/models/integrator_active.json"
ACTIVATION_TRANSACTION_ROOT="${CLOSED_LOOP_ACTIVATION_TRANSACTION_ROOT:-${AI_TRADE_DATA_DIR:-./data}/models/activation_transactions}"
ACTIVATION_TRANSACTION_DIR="${ACTIVATION_TRANSACTION_ROOT}/${RUN_ID}"
ACTIVATION_TRANSACTION_STATE_PATH="${CLOSED_LOOP_ACTIVATION_TRANSACTION_STATE_PATH:-${AI_TRADE_DATA_DIR:-./data}/models/activation_transaction.json}"
...
CLOSED_LOOP_RUNNER_LOCK_PATH="${CLOSED_LOOP_RUNNER_LOCK_PATH:-${AI_TRADE_DATA_DIR:-./data}/models/closed_loop_runner.lock}"
...
run_maker_execution_opportunity_experiment() {
  ...
  compose_cmd --profile research run --rm --entrypoint python3 ai-trade-research \
    ...
    --baseline-audit-manifest "${MAKER_OPPORTUNITY_BASELINE_AUDIT_MANIFEST}" \
...
run_cross_asset_residual_opportunity_experiment() {
  echo "[INFO] dollar-neutral cross-asset residual opportunity experiment start"
  local status=0
  compose_cmd --profile research run --rm --entrypoint python3 ai-trade-research \
    tools/run_cross_asset_residual_opportunity_experiment.py \
    --control-assessment "${MICROSTRUCTURE_CAPTURE_REPORT_PATH}" \
    --config "${CROSS_ASSET_RESIDUAL_EXPERIMENT_CONFIG}" \
    --parent-audit-manifest "${MAKER_OPPORTUNITY_BASELINE_AUDIT_MANIFEST}" \
    --audit-manifest "${CROSS_ASSET_RESIDUAL_AUDIT_MANIFEST}" \
    --output "${CROSS_ASSET_RESIDUAL_EXPERIMENT_REPORT_PATH}" \
    --research-domain development || status=$?
  if [[ -s "${CROSS_ASSET_RESIDUAL_AUDIT_MANIFEST}" ]]; then
    cp -f -- "${CROSS_ASSET_RESIDUAL_AUDIT_MANIFEST}" \
      "${CROSS_ASSET_RESIDUAL_AUDIT_SNAPSHOT_PATH}"
  elif [[ -s "${CROSS_ASSET_RESIDUAL_EXPERIMENT_REPORT_PATH}" ]]; then
    CROSS_ASSET_RESIDUAL_REPORT_PATH_VALUE="${CROSS_ASSET_RESIDUAL_EXPERIMENT_REPORT_PATH}" \
...
report_path = Path(os.environ["CROSS_ASSET_RESIDUAL_REPORT_PATH_VALUE"])
report = json.loads(report_path.read_text(encoding="utf-8"))
payload = {
    "schema_version": "cross_asset_residual_frozen_audit_unavailable_v1",
    "state": "NOT_FROZEN",
    "research_decision": str(report.get("research_decision") or "NOT_READY"),
    "reason_codes": list(report.get("reason_codes") or ["audit_not_created"]),
    "opportunity_report_sha256": hashlib.sha256(report_path.read_bytes()).hexdigest(),
...
Path(os.environ["CROSS_ASSET_RESIDUAL_AUDIT_SNAPSHOT_PATH_VALUE"]).write_text(
    json.dumps(payload, ensure_ascii=False, sort_keys=True) + "\n",
    encoding="utf-8",
)
PY
  fi
  echo "[INFO] dollar-neutral cross-asset residual opportunity experiment done"
  return "${status}"
}
...
run_cross_venue_funding_differential_experiment() {
  echo "[INFO] cross-venue perpetual funding-differential opportunity experiment start"
  local status=0
  compose_cmd --profile research run --rm --entrypoint python3 ai-trade-research \
    tools/fetch_cross_venue_funding_history.py \
    --parent-audit-manifest "${FUNDING_BASIS_CARRY_AUDIT_SNAPSHOT_PATH}" \
    --output "${CROSS_VENUE_FUNDING_HISTORY_PATH}" \
    --report "${CROSS_VENUE_FUNDING_DATA_REPORT_PATH}" \
    --symbol SOLUSDT || status=$?
  compose_cmd --profile research run --rm --entrypoint python3 ai-trade-research \
    tools/run_cross_venue_funding_differential_experiment.py \
    --history-csv "${CROSS_VENUE_FUNDING_HISTORY_PATH}" \
    --data-report "${CROSS_VENUE_FUNDING_DATA_REPORT_PATH}" \
    --parent-audit-manifest "${FUNDING_BASIS_CARRY_AUDIT_SNAPSHOT_PATH}" \
    --config "${CROSS_VENUE_FUNDING_EXPERIMENT_CONFIG}" \
    --audit-manifest "${CROSS_VENUE_FUNDING_AUDIT_MANIFEST}" \
    --output "${CROSS_VENUE_FUNDING_EXPERIMENT_REPORT_PATH}" \
    --research-domain development || status=$?
  if [[ -s "${CROSS_VENUE_FUNDING_AUDIT_MANIFEST}" ]]; then
    cp -f -- "${CROSS_VENUE_FUNDING_AUDIT_MANIFEST}" \
      "${CROSS_VENUE_FUNDING_AUDIT_SNAPSHOT_PATH}"
  elif [[ -s "${CROSS_VENUE_FUNDING_EXPERIMENT_REPORT_PATH}" ]]; then
    CROSS_VENUE_FUNDING_REPORT_PATH_VALUE="${CROSS_VENUE_FUNDING_EXPERIMENT_REPORT_PATH}" \
...
report_path = Path(os.environ["CROSS_VENUE_FUNDING_REPORT_PATH_VALUE"])
report = json.loads(report_path.read_text(encoding="utf-8"))
payload = {
    "schema_version": "cross_venue_funding_differential_frozen_audit_unavailable_v1",
    "state": "NOT_FROZEN",
    "research_decision": str(report.get("research_decision") or "NOT_READY"),
    "reason_codes": list(report.get("reason_codes") or ["audit_not_created"]),
    "opportunity_report_sha256": hashlib.sha256(report_path.read_bytes()).hexdigest(),
...
Path(os.environ["CROSS_VENUE_FUNDING_AUDIT_SNAPSHOT_PATH_VALUE"]).write_text(
    json.dumps(payload, ensure_ascii=False, sort_keys=True) + "\n",
    encoding="utf-8",
)
PY
  fi
  echo "[INFO] cross-venue perpetual funding-differential opportunity experiment done"
  return "${status}"
}

run_account_structural_economics_audit() {
  echo "[INFO] read-only account structural economics audit start"
  # This runs on the release host so credentials sourced from the protected
  # runtime env are available without injecting them into the research image.
  python3 tools/audit_account_structural_economics.py \
    --upstream-report "${CROSS_VENUE_FUNDING_EXPERIMENT_REPORT_PATH}" \
    --config "${ACCOUNT_STRUCTURAL_ECONOMICS_CONFIG}" \
    --output "${ACCOUNT_STRUCTURAL_ECONOMICS_REPORT_PATH}" \
    --private-mode auto
}
...
run_option_variance_risk_premium_sequential_payoff() {
  echo "[INFO] checksum-bound option VRP sequential payoff audit start"
  python3 tools/audit_option_vrp_sequential_payoff.py \
    --config "${OPTION_VRP_SEQUENTIAL_CONFIG}" \
    --manifest "${OPTION_VRP_SEQUENTIAL_MANIFEST}" \
    --capture-root "${OPTION_VRP_V2_CAPTURE_ROOT}" \
    --output "${OPTION_VRP_SEQUENTIAL_REPORT_PATH}"
}

run_maker_execution_learnability_experiment() {
  echo "[INFO] conservative maker execution learnability experiment start"
  compose_cmd --profile research run --rm --entrypoint python3 ai-trade-research \
    tools/run_maker_execution_learnability_experiment.py \
    --control-assessment "${MICROSTRUCTURE_CAPTURE_REPORT_PATH}" \
    --opportunity-report "${MAKER_OPPORTUNITY_EXPERIMENT_REPORT_PATH}" \
    --config "${MAKER_LEARNABILITY_EXPERIMENT_CONFIG}" \
    --output "${MAKER_LEARNABILITY_EXPERIMENT_REPORT_PATH}" \
    --research-domain development
...
write_run_manifest() {
  ...
  OPTION_VRP_SEQUENTIAL_CONFIG_VALUE="${OPTION_VRP_SEQUENTIAL_CONFIG}" \
  OPTION_VRP_SEQUENTIAL_MANIFEST_VALUE="${OPTION_VRP_SEQUENTIAL_MANIFEST}" \
  ...
  FUNDING_BASIS_CARRY_AUDIT_SNAPSHOT_PATH_VALUE="${FUNDING_BASIS_CARRY_AUDIT_SNAPSHOT_PATH}" \
  CROSS_VENUE_FUNDING_HISTORY_PATH_VALUE="${CROSS_VENUE_FUNDING_HISTORY_PATH}" \
  CROSS_VENUE_FUNDING_DATA_REPORT_PATH_VALUE="${CROSS_VENUE_FUNDING_DATA_REPORT_PATH}" \
  ...
  OPTION_VRP_SEQUENTIAL_REPORT_PATH_VALUE="${OPTION_VRP_SEQUENTIAL_REPORT_PATH}" \
...
payload = {
    ...
    "config_paths": {
        ...
        "maker_execution_opportunity_experiment": os.environ.get(
            "MAKER_OPPORTUNITY_EXPERIMENT_CONFIG_VALUE", ""
        ),
        ...
        "cross_venue_funding_differential_experiment": os.environ.get(
            "CROSS_VENUE_FUNDING_EXPERIMENT_CONFIG_VALUE", ""
        ),
        ...
        "option_variance_risk_premium_sequential_payoff": os.environ.get(
            "OPTION_VRP_SEQUENTIAL_CONFIG_VALUE", ""
        ),
        "option_variance_risk_premium_sequential_payoff_manifest": os.environ.get(
            "OPTION_VRP_SEQUENTIAL_MANIFEST_VALUE", ""
        ),
...
artifact_env_names = {
    ...
    "cross_venue_funding_history": "CROSS_VENUE_FUNDING_HISTORY_PATH_VALUE",
    "cross_venue_funding_data_report": "CROSS_VENUE_FUNDING_DATA_REPORT_PATH_VALUE",
    ...
    "option_variance_risk_premium_sequential_payoff": "OPTION_VRP_SEQUENTIAL_REPORT_PATH_VALUE",
...
build_summary() {
  ...
  if [[ -f "${MAKER_OPPORTUNITY_EXPERIMENT_REPORT_PATH}" ]]; then
    SUMMARY_ARGS+=(--maker_execution_opportunity_experiment_report "${MAKER_OPPORTUNITY_EXPERIMENT_REPORT_PATH}")
  fi
  ...
  if [[ -f "${CROSS_VENUE_FUNDING_EXPERIMENT_REPORT_PATH}" ]]; then
    SUMMARY_ARGS+=(--cross_venue_funding_differential_experiment_report "${CROSS_VENUE_FUNDING_EXPERIMENT_REPORT_PATH}")
  fi
  ...
  if [[ -f "${OPTION_VRP_SEQUENTIAL_REPORT_PATH}" ]]; then
    SUMMARY_ARGS+=(--option_variance_risk_premium_sequential_payoff_report "${OPTION_VRP_SEQUENTIAL_REPORT_PATH}")
  fi
  ...
  cat > "${RUN_META_PATH}" <<EOF
{
  ...
  "cross_venue_funding_history": "${CROSS_VENUE_FUNDING_HISTORY_PATH}",
  "cross_venue_funding_data_report": "${CROSS_VENUE_FUNDING_DATA_REPORT_PATH}",
  ...
  "option_variance_risk_premium_sequential_payoff": "${OPTION_VRP_SEQUENTIAL_REPORT_PATH}",
...
observation_report_path() {
  case "$1" in
    ...
    option_variance_risk_premium_sequential_payoff) printf '%s\n' "${OPTION_VRP_SEQUENTIAL_REPORT_PATH}" ;;
...
run_training_chain() {
  ...
  if (( RUN_REQUIRED_STEP_STATUS == 0 )); then
    ...
    run_observation_step option_variance_risk_premium_sequential_payoff run_option_variance_risk_premium_sequential_payoff
...
run_research_discovery_chain() {
  ...
  if (( RUN_REQUIRED_STEP_STATUS != 0 )); then
    ...
    skip_observation_step option_variance_risk_premium_sequential_payoff "${reason}"
...
run_assess_observation_chain() {
  if [[ "${STAGE}" == "DEPLOY" || "${STAGE}" == "SMOKE" ]]; then
    ...
    skip_observation_step option_variance_risk_premium_sequential_payoff "${skip_reason}"
