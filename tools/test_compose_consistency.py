...
class ComposeConsistencyTest(unittest.TestCase):
    ...
    def test_prod_has_closed_loop_services(self):
        ...
        self.assertIn("option-vrp-collector", self.prod_services)
    ...
    def test_option_vrp_collector_is_public_persistent_and_health_checked(self):
        ...
        self.assertIn(
            "${AI_TRADE_DATA_DIR:-/opt/ai-trade/data}:/app/data",
            self.prod_services["option-vrp-collector"],
        )
    ...
    def test_deploy_defaults_match_prod_container_names(self):
        ...
        self.assertEqual(
            prod_container_names.get("option-vrp-collector"),
            "ai-trade-option-vrp-collector",
        )
        self.assertEqual(
    ...
    def test_deploy_disk_preflight_cleans_pressure_before_service_mutation(self):
        ...
        self.assertIn(
            'DEPLOY_MIN_FREE_BYTES="${DEPLOY_MIN_FREE_BYTES:-671088640}"',
            script,
        )
        self.assertIn(
            "DEPLOY_MIN_FREE_BYTES: ${{ vars.DEPLOY_MIN_FREE_BYTES || '671088640' }}",
            workflow,
        )
        self.assertIn(
        ...
        self.assertIn(
            "DEPLOY_OPTION_VRP_CAPTURE_RETENTION_HOURS: ${{ vars.DEPLOY_OPTION_VRP_CAPTURE_RETENTION_HOURS || '240' }}",
            workflow,
        )
        self.assertIn(
        ...
        self.assertIn(
            "DEPLOY_OPTION_LIFECYCLE_V3_CAPTURE_RETENTION_HOURS: ${{ vars.DEPLOY_OPTION_LIFECYCLE_V3_CAPTURE_RETENTION_HOURS || '960' }}",
            workflow,
        )
        self.assertIn(
