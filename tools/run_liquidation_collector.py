#!/usr/bin/env python3
"""Run restartable rotating Bybit SOLUSDT all-liquidation captures."""
...
SCHEMA_VERSION = "liquidation_collector_health_v1"
...
def segment_command(*, root: pathlib.Path, duration_sec: float, public_url: str) -> tuple[Sequence[str], pathlib.Path]:
...
def run(args: argparse.Namespace) -> int:
    ...
    while args.max_segments <= 0 or completed < args.max_segments:
        ...
        command, report_path = segment_command(root=root, duration_sec=duration, public_url=args.public_url)
        started = int(time.time() * 1000)
        atomic_write_json(
            health,
            {"schema_version": SCHEMA_VERSION, "state": "capturing", "symbol": collector.SYMBOL,
        ...
        try:
            ...
            atomic_write_json(
                latest,
                {"schema_version": "liquidation_latest_segment_v1", "symbol": collector.SYMBOL,
                 "capture_schema_version": collector.SCHEMA_VERSION, "report": str(relative),
                 "report_payload": report, "completed_epoch_ms": completed_at},
            )
            atomic_write_json(
                health,
                {"schema_version": SCHEMA_VERSION, "state": "healthy", "symbol": collector.SYMBOL,
                 "capture_schema_version": collector.SCHEMA_VERSION, "segment_started_epoch_ms": started,
                 "last_success_epoch_ms": completed_at, "consecutive_failures": 0,
                 "latest_report": str(relative)},
            )
            retention.prune_capture_root(root, retention_seconds=args.retention_days * 86400, now_epoch=time.time())
            successes += 1
        except (OSError, RuntimeError, ValueError, json.JSONDecodeError, subprocess.CalledProcessError) as exc:
            failures += 1
            atomic_write_json(
                health,
                {"schema_version": SCHEMA_VERSION, "state": "degraded", "symbol": collector.SYMBOL,
                 "capture_schema_version": collector.SCHEMA_VERSION, "segment_started_epoch_ms": started,
                 "last_failure_epoch_ms": int(time.time() * 1000), "consecutive_failures": failures,
                 "error": str(exc)},
            )
...
def main() -> int:
    args = parse_args()
    if args.action == "run":
        if min(args.segment_duration_sec, args.bootstrap_segment_duration_sec, args.retention_days) <= 0:
